...
int main(void)
{
    ...
    if (pthread_pool_init(&pool1, 65536, 16) == POOL_FAIL)          // 스레드의 수가 65536 이고, 대기열의 용량이 16인 스레드풀 pool1을 생성함
        printf("pthreaad_pool_init(): 일꾼 스레드 최대 수 초과.....PASSED\n");
    else {
        printf("Error: 일꾼 스레드 갯수 검사 오류\n");
        return -1;
    }
    if (pthread_pool_init(&pool1, 16, 65536) == POOL_FAIL)         // 스레드의 수가 16이고, 대기열의 용량이 65536인 스레드풀 pool1을 생성함
    ...
    if (pthread_pool_init(&pool1, 64, 64)) {                // 스레드의 수가 64이고, 대기열의 용량이 64인 스레드풀 pool1을 생성함
        printf("Error: 스레드풀 초기화 오류\n");
        return -1;
    }
    printf("pthread_pool_init(): 완료......PASSED\n");
    if (pthread_pool_shutdown(&pool1, POOL_DISCARD)) {          // 스레드풀 pool1을 종료함, 일꾼 스레드가 현재 작업 중이면 그 작업을 마치게 함, PULL_DISCARD 이면 대기열에 새 작업이 남아 있어도 더 이상 수행하지 않고 종료함
    ...
    printf("pthread_pool_init(): 완료......PASSED\n");
    if (pthread_pool_shutdown(&pool1, POOL_COMPLETE)) {        // 스레드풀 pool1을 종료함, 일꾼 스레드가 현재 작업 중이면 그 작업을 마치게 함, PULL_COMPLETE 이면 대기열에 남아 있는 모든 작업을 종료함
        printf("Error: 스레드풀 종료 오류\n");
        return -1;
    }
    printf("pthread_pool_shutdown(): 완료......PASSED\n");
    ...
    pthread_pool_init(&pool1, 3, 10);                       // 스레드의 수가 3이고, 대기열의 용량이 10인 스레드풀 pool1을 생성함
    /*
    ...
    for (i = 0; i < NTASK; ++i) {
        num[i] = i;
        if (pthread_pool_submit(&pool1, number1, num+i, POOL_NOWAIT))       // 스레드풀 pool1에 작업을 제출함, num+i를 인자로 한 number1 함수를 수행함, POOL_NOWAIT는 대기열에 빈자리가 없으면 기다리지 않고 즉시 POOL_FULL을 리턴함
    ...
    for (i = 0; i < NTASK; ++i) {
        num[i] = i;
        if (pthread_pool_submit(&pool1, number2, num+i, POOL_WAIT)) {      // 스레드풀 pool1에 작업을 제출함, num+i를 인자로 한 number1 함수를 수행함, POOL_WAIT는 대기열에 빈자리가 생길 때까지 기다리다 제출하고 POOL_SUCCESS을 리턴함
     ...
     */
    pthread_pool_shutdown(&pool1, POOL_DISCARD);            // 스레드풀 pool1을 종료함, 일꾼 스레드가 현재 작업 중이면 그 작업을 마치게 함, PULL_DISCARD 이면 대기열에 새 작업이 남아 있어도 더 이상 수행하지 않고 종료함
    printf("......PASSED\n");
    ...
    struct foo arg[16];
    pthread_pool_init(&pool1, 8, 16);               // 스레드의 수가 8 이고, 대기열의 용량이 16인 스레드풀 pool1을 생성함
    count = 0;
    for (i = 0; i < 16; ++i) {
        arg[i].id = i;
        arg[i].num = 0x0fffffff00000000 + i*100;
        pthread_pool_submit(&pool1, find_primes, arg+i, POOL_WAIT);         // 스레드풀 pool1에 작업을 제출함, arg+i를 인자로 한 find_primes 함수를 수행함, POOL_WAIT는 대기열에 빈자리가 생길 때까지 기다리다 제출하고 POOL_SUCCESS을 리턴함
    }
    pthread_pool_shutdown(&pool1, POOL_DISCARD);        // 스레드풀 pool1을 종료함, 일꾼 스레드가 현재 작업 중이면 그 작업을 마치게 함, PULL_DISCARD 이면 대기열에 새 작업이 남아 있어도 더 이상 수행하지 않고 종료함
    printf("소수 %d개를 찾았다.\n", count);
    if (count < 18)
